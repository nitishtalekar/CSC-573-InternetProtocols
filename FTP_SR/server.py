                ...
                if computed_checksum == data[32:48] and data[48:64] == self.DATA_TYPE:
                    ...
                    if self.packet_accepted():
                        if self.SEQ_NO == int(data[:32], 2):
                            ...
                            for i in sorted(self.STORE,key=lambda l:l[0]):
                                d= i[1]
                                if self.SEQ_NO == int(d[:32],2):
                                    self.FILE_OUTPUT.write(str.encode(d[64:]))
                                    self.SEQ_NO += len(d[64:])
                        else:
                            self.STORE.append([int(data[:32],2),data])
                        header = data[:32]
